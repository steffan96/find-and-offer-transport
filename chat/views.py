...
def chat(request, receiver):
    ...
    if request.method == "GET":
        chat = ChatBox.objects.filter(
            Q(user1=request.user, user2=chat_receiver)
            | Q(user2=request.user, user1=chat_receiver)
        ).first()
        if not chat:
        ...
        try:
            if msg.sender != request.user:
                each_message = Message.objects.filter(
                    Q(chat=chat) & ~Q(sender=request.user)
                ).all()
...
class InboxView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        object_list = ChatBox.objects.filter(
